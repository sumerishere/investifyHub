...
function StartUpComp() {
    ...
    useEffect(()=>{
        async function fun(){
            const dt = await fetch(`http://localhost:8080/getByIndustry?ind=${inpvalue}`)
            ...
            <Select   placeholder='Category' onSelect={select}>
                {
                    arr1.map(data=>{
                        return (
                            <Select.Option className='se'  key={data.id} value={`${data}`}>{`${data}`}</Select.Option>
                        )
                    })
                }
            </Select>
            </div>
            <div className="bt"><button className='btn2' style={btns} onClick={reset}>Reset</button></div>
        </div>
        <div className="cont2">
        <div className="cont1">
        {
